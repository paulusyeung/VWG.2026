...
// Type: Gizmox.WebGUI.Forms.Skins.HorizontalAlignmentValueConverter
...
namespace Gizmox.WebGUI.Forms.Skins
{
  ...
  public class HorizontalAlignmentValueConverter : TypeConverter
  {
    ...
    /// </returns>
    public override bool CanConvertTo(ITypeDescriptorContext objContext, Type objDestinationType) => objDestinationType == typeof (string) || objDestinationType == typeof (int) || base.CanConvertTo(objContext, objDestinationType);
    ...
    /// true if this converter can perform the conversion; otherwise, false.
    /// </returns>
    public override bool CanConvertFrom(ITypeDescriptorContext objContext, Type objSourceType) => objSourceType == typeof (string) || objSourceType == typeof (int) || base.CanConvertFrom(objContext, objSourceType);
      ...
      switch (objValue)
      {
        ...
        case int _:
          ...
          if (!string.IsNullOrEmpty(str))
            return Enum.IsDefined(typeof (HorizontalAlignment), (object) str) ? (object) new HorizontalAlignmentValue((HorizontalAlignment) Enum.Parse(typeof (HorizontalAlignment), str, true)) : throw new Exception("Illegal horizontal alignment value.");
          break;
      }
    ...
    {
      return objDestinationType == typeof (string) && objValue is HorizontalAlignmentValue horizontalAlignmentValue ? (object) horizontalAlignmentValue.ToString() : base.ConvertTo(objContext, objCulture, objValue, objDestinationType);
    }
    ...
    {
      HorizontalAlignmentValue horizontalAlignmentValue = (HorizontalAlignmentValue) objContext.PropertyDescriptor.GetValue(objContext.Instance);
      int objPropertyValue = (int) objPropertyValues[(object) "HorizontalAlignment"];
      return Activator.CreateInstance(horizontalAlignmentValue.GetType(), (object) objPropertyValue);
    }
    ...
    {
      return TypeDescriptor.GetProperties(typeof (HorizontalAlignmentValue), arrAttributes).Sort(new string[1]
      {
        "HorizontalAlignment"
      });
    }
    ...
    public override TypeConverter.StandardValuesCollection GetStandardValues(
      ITypeDescriptorContext objContext)
    {
      return new TypeConverter.StandardValuesCollection((ICollection) Enum.GetValues(typeof (HorizontalAlignment)));
    }
  }
}
