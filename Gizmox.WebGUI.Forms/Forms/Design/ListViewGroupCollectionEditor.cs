...
// Type: Gizmox.WebGUI.Forms.Design.ListViewGroupCollectionEditor
...
namespace Gizmox.WebGUI.Forms.Design
{
  ...
  public class ListViewGroupCollectionEditor : CollectionEditor
  {
    ...
    protected override object SetItems(object objEditValue, object[] arrValues)
    {
      ...
      {
        ListViewGroupCollection viewGroupCollection = (ListViewGroupCollection) objEditValue;
        viewGroupCollection.ClearInternal();
        for (int index = 0; index < arrValues.Length; ++index)
        {
          if (arrValues[index] is ListViewGroup arrValue)
            viewGroupCollection.Add(arrValue);
        }
      }
      return objEditValue;
    }
    ...
    private void objForm_Closed(object sender, EventArgs e)
    {
      ...
      if (context == null || !(context.Instance is ListView instance))
        return;
      ListViewGroupCollection groups = instance.Groups;
      if (groups == null)
        return;
      foreach (ListViewItem listViewItem in instance.Items)
      {
        ListViewGroup group = listViewItem.Group;
