...
/// 
	...
	public class PaddingConverter : TypeConverter
	{
		...
		public override object ConvertFrom(ITypeDescriptorContext objContext, CultureInfo objCulture, object objValue)
		{
			if (objValue is string)
			{
				...
				if (!string.IsNullOrEmpty(text))
				{
					...
					if (array.Length == 1)
					{
						return new Padding(int.Parse(array[0]));
					}
					if (array.Length == 4)
					{
		...
		public override object ConvertTo(ITypeDescriptorContext objContext, CultureInfo objCulture, object objValue, Type objDestinationType)
		{
			...
			bool flag = false;
			flag |= objDestinationType == typeof(InstanceDescriptor) && objValue is Padding;
			if (flag | (objDestinationType == typeof(string) && objValue is Padding))
			{
				...
				if (objDestinationType == typeof(string))
				{
					...
					return $"{objPadding.Left}, {objPadding.Top}, {objPadding.Right}, {objPadding.Bottom}";
				}
				if (objDestinationType == typeof(InstanceDescriptor))
				{
		...
		private object ConvertToInstanceDescriptor(ITypeDescriptorContext objContext, Padding objPadding)
		{
			...
			if (objPadding.ShouldSerializeAll())
			{
				...
				arguments = new object[1] { objPadding.All };
				return new InstanceDescriptor(typeof(Padding).GetConstructor(types), arguments);
			}
			types = new Type[4]
			{
				typeof(int),
			...
			arguments = new object[4] { objPadding.Left, objPadding.Top, objPadding.Right, objPadding.Bottom };
			return new InstanceDescriptor(typeof(Padding).GetConstructor(types), arguments);
		}
		...
		{
			PropertyDescriptorCollection properties = TypeDescriptor.GetProperties(typeof(Padding), arrAttributes);
