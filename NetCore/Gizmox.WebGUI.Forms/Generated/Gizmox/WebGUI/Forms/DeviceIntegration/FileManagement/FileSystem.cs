	...
	public class FileSystem : DevicePropertyDictionary, IFileSystem
	{
		...
		public static FileSystem FromVWGEvent(FileManager objFileManager, IEvent objEvent)
		{
			if (objEvent != null)
			{
				...
				fileSystem.Name = objEvent["filesystemName"];
				if (!string.IsNullOrEmpty(objEvent["filesystemSize"]) && !string.IsNullOrEmpty(objEvent["filesystemType"]))
				{
					fileSystem.Type = (FileSystemType)int.Parse(objEvent["filesystemType"]);
					fileSystem.RequestedSize = ulong.Parse(objEvent["filesystemSize"]);
				}
				fileSystem.Root = DirectoryEntry.FromVWGEvent(objEvent, objFileManager, fileSystem);
				return fileSystem;
			}
			return null;
		}
