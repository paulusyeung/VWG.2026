...
namespace Gizmox.WebGUI.Forms.Dialogs
{
	...
	public class ListViewGroupingOptions : Form
	{
		...
		public ListViewGroupingOptions(ListView objListView)
		{
			...
			mobjItemChooser.Items = mobjGroupedColumns.SortedColumns;
			mobjItemChooser.Checked = mobjGroupedColumns.GroupingColumns;
			...
			mobjLabelHelp.Text = SR.GetString(Context.CurrentUICulture, "WGLablesGroupingInstructions");
			Text = SR.GetString(Context.CurrentUICulture, "WGLablesGroupingOptions");
		}
		...
		private void InitializeComponent()
		{
			...
			mobjLabelHelp.Text = "Check the columns that you would like to group by. Use the Move  Up and Move Down buttons to reorder grouping order.";
			...
			base.Name = "ListViewGroupingOptions";
		...
		private void mobjButtonOK_Click(object sender, EventArgs e)
		{
			...
			{
				item.GroupByPosition = 1000;
			}
			int num = 0;
			foreach (ColumnHeader item2 in mobjItemChooser.Checked)
			{
				item2.GroupBy = true;
				item2.GroupByPosition = num;
				num++;
			}
			foreach (ColumnHeader item3 in mobjItemChooser.Items)
			{
				if (item3.GroupByPosition == 1000)
				{
					item3.GroupBy = false;
					item3.GroupByPosition = num;
				}
				num++;
			}
			Close();
