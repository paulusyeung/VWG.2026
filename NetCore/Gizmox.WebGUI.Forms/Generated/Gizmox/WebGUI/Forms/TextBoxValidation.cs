...
/// 
	...
	public class TextBoxValidation
	{
		...
		/// </summary>
		/// The character validation expression.</value>
		...
		public string Expression => mstrValueValidationExpression;

		/// 
		/// Gets the value validation expression.
		/// </summary>
		/// The value validation expression.</value>
		public string ValueValidationExpression => mstrValueValidationExpression;
		...
		public string Mask => mstrCharacterValidationMask;

		/// 
		/// Gets the character validation mask.
		/// </summary>
		/// The character validation mask.</value>
		public string CharacterValidationMask => mstrCharacterValidationMask;
		...
		public string Message => mstrInValidateMessage;

		/// 
		/// Gets the in validate message.
		/// </summary>
		/// The in validate message.</value>
		public string InValidateMessage => mstrInValidateMessage;
		...
		public static TextBoxValidation FloatValidator
		{
			get
			{
				...
				return new TextBoxValidation("String(value).match(/^-{0,1}[0-9]*([" + DecimalSeparator + "][0-9]+){0,1}$/)", SR.GetString(currentUICulture, "WGLablesFloatRequiered"), "0-9\\" + DecimalSeparator + "\\|\\x1B");
			}
		}
		...
		/// </summary>
		/// </value>
		public static TextBoxValidation FloatMaskValidator => new TextBoxValidation("", "", "0-9\\" + DecimalSeparator + "\\|\\x1B");
		...
		public static TextBoxValidation EmailValidator
		{
			get
			{
				...
				string arg = "^(([\\w-]+\\.)+[\\w-]+|([a-zA-Z]{1}|[\\w-]{2,}))@((([0-1]?[0-9]{1,2}|25[0-5]|2[0-4][0-9])\\.([0-1]?[0-9]{1,2}|25[0-5]|2[0-4][0-9])\\.([0-1]?[0-9]{1,2}|25[0-5]|2[0-4][0-9])\\.([0-1]?[0-9]{1,2}|25[0-5]|2[0-4][0-9])){1}|([a-zA-Z]+[\\w-]+\\.)+[a-zA-Z]{2,4})$";
				return new TextBoxValidation($"String(value).match(/{arg}/)", SR.GetString(currentUICulture, "WGLablesEmailRequiered"));
			}
		}
		...
		public static TextBoxValidation CreateIntegerValidator(int intMinValue, int intMaxValue)
		{
			...
			return new TextBoxValidation($"String(value).match(/^[0-9]*$/)&& (value>={intMinValue} && value<={intMaxValue})", SR.GetString(currentUICulture, "WGLablesIntegerRangeRequiered", intMinValue, intMaxValue), "0-9|\\x1B");
		}
		...
		public static TextBoxValidation CreateFloatValidator(double intMinValue, double intMaxValue)
		{
			...
			return new TextBoxValidation($"(value>={intMinValue} && value<={intMaxValue})", SR.GetString(currentUICulture, "WGLablesFloatRangeRequiered", intMinValue, intMaxValue));
		}
	}
}
