...
namespace Gizmox.WebGUI.Forms.PropertyGridInternal
{
	...
	internal class ArrayElementGridEntry : GridEntry
	{
		...
		protected int mintIndex
		{
			get
			{
				return GetValue(mintIndexProperty);
			}
			set
			{
				SetValue(mintIndexProperty, value);
			}
		}
		...
		public override string PropertyLabel => "[" + mintIndex.ToString(CultureInfo.CurrentCulture) + "]";

		public override Type PropertyType => base.mobjParentGridEntry.PropertyType.GetElementType();
		...
		public override object PropertyValue
		{
			get
			{
				...
				return ((Array)valueOwner).GetValue(mintIndex);
			}
			set
			{
				object valueOwner = GetValueOwner();
				((Array)valueOwner).SetValue(value, mintIndex);
			}
		}
		...
		{
			mintIndex = intIndex;
			SetState(256, (objParentGridEntry.Flags & 0x100) != 0 || objParentGridEntry.ForceReadOnly);
		}
	}
}
