...
[Serializable]
	internal class AttributeTypeSorter : IComparer
	{
		...
		public int Compare(object obj1, object obj2)
		{
			...
			return string.Compare(GetTypeIdString(attribute), GetTypeIdString(attribute2), ignoreCase: false, CultureInfo.InvariantCulture);
		}
		...
		private static string GetTypeIdString(Attribute objAttribute)
		{
			...
			if (mobjTypeIds == null)
			{
				mobjTypeIds = new Hashtable();
				text = null;
			}
			else
			{
				text = mobjTypeIds[typeId] as string;
			}
			if (text == null)
			{
				text = typeId.ToString();
				mobjTypeIds[typeId] = text;
			}
			return text;
		}
	}
