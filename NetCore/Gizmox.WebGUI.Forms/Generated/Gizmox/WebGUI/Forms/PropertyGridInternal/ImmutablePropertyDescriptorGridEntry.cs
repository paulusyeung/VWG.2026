...
namespace Gizmox.WebGUI.Forms.PropertyGridInternal
{
	...
	internal class ImmutablePropertyDescriptorGridEntry : PropertyDescriptorGridEntry
	{
		...
		protected override bool IsPropertyReadOnly => ShouldRenderReadOnly;
		...
		public override object PropertyValue
		{
			...
			set
			{
				...
				GridEntry instanceParentGridEntry = InstanceParentGridEntry;
				...
				{
					if (base.mobjPropertyInfo.Name != null && base.mobjPropertyInfo.Name.Equals(properties[i].Name))
					{
				...
				catch (Exception ex)
				{
					...
					{
						throw new TargetInvocationException(SR.GetString("ExceptionCreatingObject", InstanceParentGridEntry.PropertyType.FullName, ex.ToString()), ex);
					}
					throw;
				}
				if (obj != null)
		...
		{
			return ParentGridEntry.NotifyValue(intType);
		}
	}
}
