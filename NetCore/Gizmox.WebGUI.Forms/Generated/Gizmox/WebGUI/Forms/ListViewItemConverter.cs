...
/// 
	...
	public class ListViewItemConverter : ExpandableObjectConverter
	{
		...
		private object ConvertToInstanceDescriptor(ITypeDescriptorContext objContext, object objValue)
		{
			...
			string[] array = new string[listViewItem.SubItems.Count];
			...
			if (listViewItem.ImageIndex == -1 && listViewItem.SubItems.Count <= 1)
			{
				...
				constructor = typeof(ListViewItem).GetConstructor(types);
				if (constructor != null)
				{
					object[] arguments = new object[1] { listViewItem.Text };
			...
			if (listViewItem.SubItems.Count <= 1)
			{
				...
				if (constructor != null)
				{
					object[] arguments = new object[2] { listViewItem.Text, listViewItem.ImageIndex };
					return new InstanceDescriptor(constructor, arguments, isComplete: false);
				}
			}
			types = new Type[2]
			...
			if (constructor != null)
			{
				object[] arguments = new object[2] { array, listViewItem.ImageIndex };
				return new InstanceDescriptor(constructor, arguments, isComplete: false);
			}
			return null;
		}
