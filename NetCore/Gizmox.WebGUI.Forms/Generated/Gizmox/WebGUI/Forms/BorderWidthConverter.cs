	...
	public class BorderWidthConverter : TypeConverter
	{
		...
		public override object ConvertTo(ITypeDescriptorContext objContext, CultureInfo objCulture, object objValue, Type objDestinationType)
		{
			...
			if (objDestinationType == typeof(string) && objValue is BorderWidth)
			{
				...
				{
					return borderWidth.All.ToString();
				}
				return $"{borderWidth.Left}, {borderWidth.Top}, {borderWidth.Right}, {borderWidth.Bottom}";
			}
		...
		private object ConvertToInstanceDescriptor(ITypeDescriptorContext objContext, object objValue)
		{
			...
			if (borderWidth.ShouldSerializeAll())
			{
				...
				arguments = new object[1] { borderWidth.All };
				return new InstanceDescriptor(typeof(BorderWidth).GetConstructor(types), arguments);
			}
			types = new Type[4]
			{
				typeof(int),
			...
			arguments = new object[4] { borderWidth.Left, borderWidth.Top, borderWidth.Right, borderWidth.Bottom };
			return new InstanceDescriptor(typeof(BorderWidth).GetConstructor(types), arguments);
		}
			...
			if (objValue is string)
			{
				...
				if (!string.IsNullOrEmpty(text))
				{
					try
					{
						...
						if (array.Length == 1)
						{
							return new BorderWidth(int.Parse(array[0]));
						}
						if (array.Length == 4)
						{
							return new BorderWidth(int.Parse(array[0]), int.Parse(array[1]), int.Parse(array[2]), int.Parse(array[3]));
						}
		...
		public override object CreateInstance(ITypeDescriptorContext objContext, IDictionary objPropertyValues)
		{
			...
			int num = (int)objPropertyValues["Left"];
			int num2 = (int)objPropertyValues["Top"];
			int num3 = (int)objPropertyValues["Right"];
			int num4 = (int)objPropertyValues["Bottom"];
			if (num == num3 && num3 == num2 && num2 == num4)
			{
				return new BorderWidth(num4);
			}
			return new BorderWidth(num, num2, num3, num4);
		}
		...
		{
			PropertyDescriptorCollection properties = TypeDescriptor.GetProperties(typeof(BorderWidth), arrAttributes);
			string[] names = new string[4] { "Left", "Top", "Right", "Bottom" };
