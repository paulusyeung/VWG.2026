...
namespace Gizmox.WebGUI.Forms.Design
{
	...
	public class VisualTemplateEditor : WebUITypeEditor
	{
		...
		public override void EditValue(ITypeDescriptorContext objContext, IServiceProvider objProvider, object objValue, WebUITypeEditorHandler objHandler)
		{
			...
			Type type = mobjComponent.GetType();
			if (mobjComponent is ProxyComponent { SourceComponent: not null } proxyComponent)
			{
				type = proxyComponent.SourceComponent.GetType();
				mobjComponent = proxyComponent.SourceComponent;
			}
			...
			{
				IEmulatorForm emulatorForm = contextParams.EmulatorForm;
				if (emulatorForm != null)
				{
					strDeviceId = emulatorForm.CurrentBrowserInfo.BrowserId;
				}
			}
			VisualTemplateControlDialog visualTemplateControlDialog = new VisualTemplateControlDialog(type, objCurrentControlVisualTemplate, mobjComponent, strDeviceId);
			visualTemplateControlDialog.Closed += objVisualizerControlDialog_Closed;
			mobjHandler = objHandler;
			if (objProvider.GetService(typeof(IWebUIEditorService)) is IWebUIEditorService webUIEditorService)
			{
				webUIEditorService.ShowDialog(visualTemplateControlDialog);
			}
		}
		...
		{
			VisualTemplateControlDialog visualTemplateControlDialog = (VisualTemplateControlDialog)sender;
			if (visualTemplateControlDialog.DialogResult == DialogResult.OK && mobjHandler != null)
			{
				mobjHandler(GetPropertyValueFromEditorValue(visualTemplateControlDialog.VisualTemplate));
				if (mobjComponent != null)
				{
