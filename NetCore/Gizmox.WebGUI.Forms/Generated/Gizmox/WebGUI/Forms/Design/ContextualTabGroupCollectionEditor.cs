...
namespace Gizmox.WebGUI.Forms.Design
{
	...
	public class ContextualTabGroupCollectionEditor : CollectionEditor
	{
		...
		protected override object SetItems(object objEditValue, object[] arrValues)
		{
			...
			{
				ContextualTabGroupCollection contextualTabGroupCollection = (ContextualTabGroupCollection)objEditValue;
				contextualTabGroupCollection.ClearInternal();
				for (int i = 0; i < arrValues.Length; i++)
				{
					if (arrValues[i] is ContextualTabGroup objGroup)
					{
						contextualTabGroupCollection.Add(objGroup);
					}
				}
			}
			return objEditValue;
		...
		private void objForm_Closed(object sender, EventArgs e)
		{
			...
			if (context == null || !(context.Instance is TabControl { ContextualTabGroups: { } contextualTabGroups } tabControl))
			{
				return;
			}
