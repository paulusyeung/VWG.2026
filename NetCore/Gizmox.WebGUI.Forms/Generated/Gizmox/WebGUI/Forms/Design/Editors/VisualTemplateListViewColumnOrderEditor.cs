...
/// 
	...
	public class VisualTemplateListViewColumnOrderEditor : WebUITypeEditor
	{
		...
		{
			VisualTemplate visualTemplate = objContext.Instance as VisualTemplate;
			...
			if (visualTemplate != null && propertyGrid != null)
			{
				ListView objListView = propertyGrid.Tag as ListView;
				string strCurrentColumnOrderAppearance = objValue as string;
				VisualTemplateListViewColumnControlDialog visualTemplateListViewColumnControlDialog = new VisualTemplateListViewColumnControlDialog(objListView, strCurrentColumnOrderAppearance);
				visualTemplateListViewColumnControlDialog.Closed += objVisualTemplateListViewColumnControlDialog_Closed;
				mobjHandler = objHandler;
				if (objProvider.GetService(typeof(IWebUIEditorService)) is IWebUIEditorService webUIEditorService)
				{
					webUIEditorService.ShowDialog(visualTemplateListViewColumnControlDialog);
				}
			}
		}
		...
		{
			VisualTemplateListViewColumnControlDialog visualTemplateListViewColumnControlDialog = (VisualTemplateListViewColumnControlDialog)sender;
			if (visualTemplateListViewColumnControlDialog.DialogResult == DialogResult.OK && mobjHandler != null)
			{
				mobjHandler(GetPropertyValueFromEditorValue(visualTemplateListViewColumnControlDialog.ListViewColumnOrder));
			}
		}
