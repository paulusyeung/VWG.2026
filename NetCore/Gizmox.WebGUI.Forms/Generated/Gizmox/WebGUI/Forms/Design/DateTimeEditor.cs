	...
	public class DateTimeEditor : WebUITypeEditor
	{
		...
		internal class DateTimeEditorDropDown : Form
		{
			...
			public DateTime Value
			{
				...
				set
				{
					mobjMonthCalendar.Value = (mobjDate = value);
				}
			}
			...
			{
				mobjMonthCalendar = new MonthCalendar();
				SuspendLayout();
				mobjMonthCalendar.Dock = DockStyle.Fill;
				mobjMonthCalendar.DateChanged += mobjMonthCalendar_ValueChanged;
				base.Controls.Add(mobjMonthCalendar);
				base.Size = new Size(175, 154);
				base.ClientSize = new Size(175, 154);
			...
			{
				mobjDate = mobjMonthCalendar.Value;
		...
		{
			DateTimeEditorDropDown dateTimeEditorDropDown = new DateTimeEditorDropDown();
			dateTimeEditorDropDown.Value = (DateTime)GetEditorValueFromPropertyValue(objValue);
			dateTimeEditorDropDown.Closed += objDateTimeDialog_Closed;
			mobjHandler = objHandler;
			if (objProvider.GetService(typeof(IWebUIEditorService)) is IWebUIEditorService webUIEditorService)
			{
				webUIEditorService.ShowDropDown(dateTimeEditorDropDown);
			}
		}

		private void objDateTimeDialog_Closed(object sender, EventArgs e)
		{
			DateTimeEditorDropDown dateTimeEditorDropDown = (DateTimeEditorDropDown)sender;
			if (dateTimeEditorDropDown.IsCompleted)
			{
				object obj = null;
				try
				{
					obj = GetPropertyValueFromEditorValue(dateTimeEditorDropDown.Value);
				}
