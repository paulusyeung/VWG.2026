...
namespace Gizmox.WebGUI.Forms.Design
{
	...
	public class ResourceHandleEditor : WebUITypeEditor
	{
		...
		{
			ServerOpenFileDialog serverOpenFileDialog = new ServerOpenFileDialog();
			serverOpenFileDialog.Closed += objServerOpenFileDialog_Closed;
			mobjHandler = objHandler;
			if (objProvider.GetService(typeof(IWebUIEditorService)) is IWebUIEditorService webUIEditorService)
			{
				webUIEditorService.ShowDialog(serverOpenFileDialog);
			}
		}
		...
		{
			if (sender is ServerOpenFileDialog { DialogResult: DialogResult.OK } serverOpenFileDialog && mobjHandler != null)
			{
				mobjHandler(GetPropertyValueFromEditorValue(GetResourceName(serverOpenFileDialog.FileName)));
			}
		}
		...
		{
			string directory = Global.Context.Config.GetDirectory("Icons");
			string directory2 = Global.Context.Config.GetDirectory("Images");
			if (strFileName.ToLower(CultureInfo.InvariantCulture).StartsWith(directory.ToLower(CultureInfo.InvariantCulture)))
			{
				return new IconResourceHandle(strFileName.Remove(0, directory.Length + 1).Replace("\\", "."));
			}
			if (strFileName.ToLower(CultureInfo.InvariantCulture).StartsWith(directory2.ToLower(CultureInfo.InvariantCulture)))
			{
				return new ImageResourceHandle(strFileName.Remove(0, directory2.Length + 1).Replace("\\", "."));
			}
			return null;
		}
	}
