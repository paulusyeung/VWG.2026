	...
	public class FramePartStyleValue : SkinMultiValue
	{
		...
		protected virtual ImageResourceReference DefaultBackgroundImage
		{
			get
			{
				...
				{
					return ((FramePartStyleValue)base.Defaults).GetValue("BackgroundImage", ImageResourceReference.Empty);
				}
		...
		public override string GetValue(IContext objContext, SkinValueDefinition objValueDefinition)
		{
			if (HasBackgroundImage)
			{
				...
				return $"background-image:url({BackgroundImage.GetValue(objContext, objValueDefinition)}){importantDeclarationValue};";
			}
			return string.Empty;
		}
	}
