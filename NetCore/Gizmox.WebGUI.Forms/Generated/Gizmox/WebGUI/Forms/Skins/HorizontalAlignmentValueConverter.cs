...
/// 
	...
	public class HorizontalAlignmentValueConverter : TypeConverter
	{
		...
		public override object ConvertFrom(ITypeDescriptorContext objContext, CultureInfo objCulture, object objValue)
		{
			if (objValue is string || objValue is int)
			{
				...
				if (!string.IsNullOrEmpty(value))
				{
					...
					{
						return new HorizontalAlignmentValue((HorizontalAlignment)Enum.Parse(typeof(HorizontalAlignment), value, ignoreCase: true));
					}
		...
		{
			if (objDestinationType == typeof(string) && objValue is HorizontalAlignmentValue horizontalAlignmentValue)
			{
				return horizontalAlignmentValue.ToString();
			}
		...
		public override object CreateInstance(ITypeDescriptorContext objContext, IDictionary objPropertyValues)
		{
			...
			int num = (int)objPropertyValues["HorizontalAlignment"];
			return Activator.CreateInstance(horizontalAlignmentValue.GetType(), num);
		}
		...
		{
			PropertyDescriptorCollection properties = TypeDescriptor.GetProperties(typeof(HorizontalAlignmentValue), arrAttributes);
			string[] names = new string[1] { "HorizontalAlignment" };
