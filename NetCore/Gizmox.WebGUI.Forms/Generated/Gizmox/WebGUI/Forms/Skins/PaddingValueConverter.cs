	...
	public class PaddingValueConverter : TypeConverter
	{
		...
		public override object ConvertFrom(ITypeDescriptorContext objContext, CultureInfo objCulture, object objValue)
		{
			if (objValue is string)
			{
				...
				if (!string.IsNullOrEmpty(text))
				{
					...
					if (array.Length == 1)
					{
						return new PaddingValue(new Padding(int.Parse(array[0])));
					}
					if (array.Length == 4)
					{
						return new PaddingValue(new Padding(int.Parse(array[0]), int.Parse(array[1]), int.Parse(array[2]), int.Parse(array[3])));
					}
				}
			}
		...
		public override object ConvertTo(ITypeDescriptorContext objContext, CultureInfo objCulture, object objValue, Type objDestinationType)
		{
			if (objDestinationType == typeof(string) && objValue is PaddingValue paddingValue)
			{
				...
				{
					return paddingValue.All.ToString();
				}
				return $"{paddingValue.Left}, {paddingValue.Top}, {paddingValue.Right}, {paddingValue.Bottom}";
			}
