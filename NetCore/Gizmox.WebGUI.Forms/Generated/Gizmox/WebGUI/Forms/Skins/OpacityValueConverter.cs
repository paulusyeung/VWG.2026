...
namespace Gizmox.WebGUI.Forms.Skins
{
	...
	public class OpacityValueConverter : TypeConverter
	{
		...
		{
			if (objDestinationType == typeof(string) && objValue is OpacityValue opacityValue)
			{
		...
		public override object CreateInstance(ITypeDescriptorContext objContext, IDictionary objPropertyValues)
		{
			...
			int num = (int)objPropertyValues["Opacity"];
			return Activator.CreateInstance(opacityValue.GetType(), num);
		}
		...
		{
			PropertyDescriptorCollection properties = TypeDescriptor.GetProperties(typeof(OpacityValue), arrAttributes);
			string[] names = new string[1] { "Opacity" };
