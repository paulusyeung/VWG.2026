...
namespace Gizmox.WebGUI.Forms
{
	...
	public abstract class ProxyNavigationCommand : ProxyAction
	{
		...
		/// 
		/// Initializes a new instance of the <see cref="T:Gizmox.WebGUI.Forms.ProxyNavigationCommand" /> class.
		/// </summary>
		...
		public override void Execute()
		{
			...
			{
				ProxyControl targetNavigationControl = TargetNavigationControl;
				if (targetNavigationControl != null && targetNavigationControl.SourceComponent is INavigationControl objINavigationControl)
				{
					flag = DoNavigation(objINavigationControl);
				}
			}
			...
			{
				List<object> objList = new List(((IFormResolver)context).AccessibleForms);
		...
		protected override PropertyDescriptorCollection GetFilteredCustomProperties(PropertyDescriptorCollection objPropertyDescriptorCollection)
		{
			...
			foreach (PropertyDescriptor item in objPropertyDescriptorCollection)
			{
				...
				if (name == "TargetNavigationControl")
				{
					flag = NavigationCommandTarget != NavigationCommandTarget.Form;
				}
				if (flag)
				{
					list.Add(item);
		...
		{
			return TargetNavigationControl != null && NavigationCommandTarget != NavigationCommandTarget.Form;
		}
	}
}
