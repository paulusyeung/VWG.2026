...
/// 
	...
	/// </summary>
	[Serializable]
	[WebCollectionEditorItemTypeName("Navigate Previous")]
	public class ProxyNavigationCommandPrevious : ProxyNavigationCommand
	{
		/// 
		/// Initializes a new instance of the <see cref="T:Gizmox.WebGUI.Forms.ProxyNavigationCommandPrevious" /> class.
		/// </summary>
		...
		{
			return objINavigationControl?.MovePrevious() ?? false;
		}
		...
		public override bool DoNavigation(IContext objContext, List<object> objList, Form objForm)
		{
			...
			{
				int num = objList.IndexOf(objForm);
				if (num > 0 && num <= objList.Count - 1)
				{
					long iD = ((Form)objList[num - 1]).ID;
					if (iD > 0 && ((ISessionRegistry)objContext).GetRegisteredComponent(iD) is Form form)
					{
						form.ActivateForm(blnUpdatePosition: true);
						return true;
					}
				}
			}
