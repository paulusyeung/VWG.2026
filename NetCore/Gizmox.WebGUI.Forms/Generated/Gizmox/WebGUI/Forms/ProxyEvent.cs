...
namespace Gizmox.WebGUI.Forms
{
	...
	public class ProxyEvent : ProxyAction
	{
		...
		/// 
		/// Initializes a new instance of the <see cref="T:Gizmox.WebGUI.Forms.ProxyEvent" /> class.
		/// </summary>
		public ProxyEvent()
		{
		}
		...
		public override void Execute()
		{
			...
			string targetComponentUniqueID = TargetComponentUniqueID;
			if (string.IsNullOrEmpty(targetComponentUniqueID))
			{
				return;
			}
			ProxyComponent actionOwner = base.ActionOwner;
			if (actionOwner == null || !(context.ActiveForm is Form objSourceComponent))
			{
				return;
			}
			Component componentByUniqueId = actionOwner.GetComponentByUniqueId(objSourceComponent, targetComponentUniqueID);
			...
			MethodInfo method = type.GetMethod($"On{defaultEventAttribute.Name}", BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
			if (!(method != null))
			{
				return;
			}
			...
			if (parameters != null)
			{
				...
				foreach (ParameterInfo parameterInfo in array)
				{
					...
					object typeDefaultValue = CommonUtils.GetTypeDefaultValue(parameterType);
					arrayList.Add(typeDefaultValue);
				}
			}
			method.Invoke(componentByUniqueId, arrayList.ToArray());
		}
