...
namespace Gizmox.WebGUI.Forms
{
	...
	public class TableLayoutPanelCellPositionTypeConverter : TypeConverter
	{
		...
		public override object ConvertFrom(ITypeDescriptorContext objContext, CultureInfo objCulture, object objValue)
		{
			...
			{
				array2[i] = (int)converter.ConvertFromString(objContext, objCulture, array[i]);
			}
			if (array2.Length != 2)
			{
				throw new ArgumentException(SR.GetString("TextParseFailedFormat", text, "column, row"));
			}
		...
		{
			TableLayoutPanelCellPosition tableLayoutPanelCellPosition = (TableLayoutPanelCellPosition)objValue;
			return new InstanceDescriptor(typeof(TableLayoutPanelCellPosition).GetConstructor(new Type[2]
			{
				typeof(int),
				typeof(int)
			}), new object[2] { tableLayoutPanelCellPosition.Column, tableLayoutPanelCellPosition.Row });
		}
		...
		{
			return TypeDescriptor.GetProperties(typeof(TableLayoutPanelCellPosition), arrAttributes).Sort(new string[2] { "Column", "Row" });
		}
