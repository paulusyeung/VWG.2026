...
namespace Gizmox.WebGUI.Forms
{
	...
	public class TabPageCollection : ICollection, IEnumerable, IList, IComparer
	{
		...
		private class TabIndexComparer : IComparer
		{
			...
			{
				TabPage tabPage = objX as TabPage;
		...
		/// The owner.
		/// </value>
		protected TabControl Owner => mobjTabControl;
		...
		public bool IsSynchronized => ((ICollection)mobjTabControl.Controls).IsSynchronized;
		...
		[Browsable(false)]
		public int Count => mobjTabControl.Controls.Count;
		...
		public object SyncRoot => ((ICollection)mobjTabControl.Controls).SyncRoot;

		/// 
		/// Gets or sets a <see cref="T:Gizmox.WebGUI.Forms.TabPage"></see> in the collection.
		/// </summary>
		/// The <see cref="T:Gizmox.WebGUI.Forms.TabPage"></see> at the specified index.</returns>
		...
		public TabPage this[int index] => (TabPage)mobjTabControl.Controls[index];
		...
		/// </summary>
		/// An <see cref="T:System.Collections.IEnumerator"></see> for the <see cref="T:Gizmox.WebGUI.Forms.TabPageCollection"></see>.</returns>
		...
		/// </summary>
		/// The zero-based index of the tab page; -1 if it cannot be found.</returns>
		...
		{
			TabPage tabPage = objTabPage as TabPage;
			...
			if (Count == 0)
			{
				mobjTabControl.Controls.Add(tabPage);
				return;
			}
			...
			try
			{
				...
				{
					TabPage tabPage2 = (TabPage)enumerator.Current;
					hashtable.Add(tabPage2.GetHashCode(), tabPage2.TabIndex);
					tabPage2.TabIndex = IndexOf(tabPage2);
				}
			}
			finally
			{
		...
		protected ArrayList GetSortedTabPageListFromCurrent(bool blnSortByIndex)
		{
			...
			try
			{
				while (enumerator.MoveNext())
				{
					TabPage value = (TabPage)enumerator.Current;
					arrayList.Add(value);
				}
			}
			finally
		...
		protected void AdvanceTabIndexFromIndex(ref ArrayList objTabPageList, ushort usrFromIndex, ushort usrAdvanceBy)
		{
			...
			{
				if (objTabPageList[num] is TabPage tabPage2)
				{
		...
		/// </summary>
		/// true if the specified object is a <see cref="T:Gizmox.WebGUI.Forms.TabPage"></see> in the collection; otherwise, false.</returns>
		...
		/// </summary>
		/// The zero-based index if page is a <see cref="T:Gizmox.WebGUI.Forms.TabPage"></see> in the collection; otherwise -1.</returns>
		...
		int IList.IndexOf(object objPage)
		{
			...
			{
				return IndexOf((TabPage)objPage);
			}
			return -1;
		}
		...
		/// </summary>
		/// The position into which the <see cref="T:Gizmox.WebGUI.Forms.TabPage"></see> was inserted.</returns>
		...
		{
			TabPage tabPage = objArgFirstTP as TabPage;
			TabPage tabPage2 = objArgSecondTP as TabPage;
			if (tabPage == null || tabPage2 == null)
			{
				return 0;
			}
			return IndexOf(tabPage).CompareTo(IndexOf(tabPage2));
		}
	}
}
