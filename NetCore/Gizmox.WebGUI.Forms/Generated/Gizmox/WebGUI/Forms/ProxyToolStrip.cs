...
namespace Gizmox.WebGUI.Forms
{
	...
	public class ProxyToolStrip : ProxyControl
	{
		...
		private void RegisterEvents(Component objComponent)
		{
			...
			{
				toolStrip.ItemAdded += objToolStrip_ItemAdded;
				toolStrip.ItemRemoved += objToolStrip_ItemRemoved;
			}
		}
		...
		private void objToolStrip_ItemRemoved(object sender, ToolStripItemEventArgs e)
		{
			...
			if (item != null)
			{
				ProxyComponent childProxyComponent = GetChildProxyComponent(item);
		...
		private void objToolStrip_ItemAdded(object sender, ToolStripItemEventArgs e)
		{
			...
			if (item != null)
			{
				ProxyComponent proxyComponent = CreateProxyByComponent(item);
		...
		public override void Render(IContext objContext, IResponseWriter objWriter, long lngRequestID)
		{
			foreach (ProxyComponent component in Components)
			{
				...
				if (component is ProxyToolStripControlHost proxyToolStripControlHost)
				{
					proxyToolStripControlHost.AttachProxy();
				}
			}
			...
			foreach (ProxyComponent component2 in Components)
			{
				...
				if (component2 is ProxyToolStripControlHost proxyToolStripControlHost2)
				{
					proxyToolStripControlHost2.DeAttachProxy();
				}
			}
		}
	}
