...
/// 
	...
	public class FormReference : IFormLink, ILink
	{
		...
		/// </summary>
		private static Config mobjConfig;
		...
		/// </summary>
		Type IFormLink.FormType => mobjType;

		/// 
		/// Gets the form instance id
		/// </summary>
		string IFormLink.FormInstance => mobjInstance.ToString();

		/// 
		/// Gets the form application name
		/// </summary>
		string IFormLink.FormApplication => mstrApplication;
		...
		public FormReference(string strApplication)
		{
			...
			string application = mobjConfig.GetApplication(strApplication.ToLower(CultureInfo.InvariantCulture));
			if (application == null)
			{
				throw new ArgumentException($"Cound not find application definition for '{strApplication}'.", "application");
			}
			mobjType = Type.GetType(application, throwOnError: false);
			if (mobjType == null)
			{
				throw new ArgumentException($"Cound not find application definition for '{strApplication}'.", "application");
			}
			mstrUrl = $"{strApplication}{mobjConfig.DynamicExtension}";
