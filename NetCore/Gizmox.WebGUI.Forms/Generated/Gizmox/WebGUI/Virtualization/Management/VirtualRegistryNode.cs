...
/// 
	...
	internal class VirtualRegistryNode : ServerExplorerNode
	{
		...
		{
			string[] array = objRegistryKey.Name.Split('\\');
		...
		{
			if (!(base.Tag is VirtualRegistryKey))
			{
				return;
			}
			int num = 0;
			if (!(base.Tag is VirtualRegistryKey virtualRegistryKey))
			{
				return;
			}
			VirtualRegistryKey[] subKeys = virtualRegistryKey.GetSubKeys();
			VirtualRegistryKey[] array = subKeys;
			foreach (VirtualRegistryKey objKey in array)
			{
				base.Nodes.Add(new VirtualRegistryNode(objKey));
				num++;
				if (num > 100)
				{
					break;
		...
		protected override void LoadItems(ListView.ListViewItemCollection objItems)
		{
			...
			string[] valueNames = virtualRegistryKey.GetValueNames();
			foreach (string text in valueNames)
			{
				object value = virtualRegistryKey.GetValue(text);
				string text2 = text;
				if (text2 == "")
				{
					text2 = "default";
				}
				ListViewItem listViewItem = objItems.Add(text2);
				listViewItem.Image = new IconResourceHandle("VirtualRegistryKey.gif");
				if (value != null)
				{
