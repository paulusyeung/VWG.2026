...
namespace Gizmox.WebGUI.Virtualization.Management
{
	...
	internal class SessionNode : ServerExplorerNode
	{
		...
		{
			base.Label = "Registery";
			base.Tag = "Registery";
			base.Image = new IconResourceHandle("file.gif");
		}
		...
		{
			ISessionRegistry sessionRegistry = Global.Context.Session as ISessionRegistry;
			foreach (DictionaryEntry item in sessionRegistry)
			{
				if (item.Value is IRegisteredComponent registeredComponent)
				{
					ListViewItem listViewItem = objItems.Add(registeredComponent.ID.ToString());
					listViewItem.SubItems.Add(registeredComponent.GetType().ToString());
				}
			}
		}
	}
