    ...
    internal sealed class FullTextEngine : IDisposable
	{
		...
		public DataTable Search(string search, int MaxHits, bool partialMatches, bool titleOnly)
		{
			if(CanSearch)
			{
                ...
                ArrayList hitsHelper = new ArrayList();

				DataTable hits = GetCreateHitsTable();

				string[] words = searchString.Split(new char[] {' '});

				for(int i=0; i<words.Length; i++)
				{
					bRet &= SearchSingleWord(words[i], MaxHits, partialMatches, titleOnly, hitsHelper);
				...
				if(bRet && IsQuoted)
				{
                    FinalizeQuoted(search, hitsHelper);
				}
				...
				if(bRet)
				{
					...
					if( MaxHits < 0)
					{
                        nhCount = hitsHelper.Count;
					}

                    if (nhCount > hitsHelper.Count)
                        nhCount = hitsHelper.Count;

					// create hits datatable
					for(int i=nhCount; i > 0; i--)
					{
                        HitHelper curHlp = (HitHelper)(hitsHelper[i - 1]);

						DataRow newRow = hits.NewRow();
                        ...
                        hits.Rows.Add(newRow);
					}
				}
				return bRet?hits:null;
			}
		...
		/// ( if this is true a search of 'support' will match 'supports', otherwise not )</param>
		/// <param name="titleOnly">true if only search in titles</param>
        /// <param name="hits">Added to allow multi user enviroment</param>
        ...
        private void DecodeWCL(byte[] wclBytes, int MaxHits, string word, ArrayList hitsHelper)
		{
			...
			while(nBitIdx < wclBits.Length)
			{
				...
				// Record hit
                HitHelper hitObj = DocumentHit(nDocIdx, hitsHelper);
		...
		{
			foreach(HitHelper curObj in hitsHelper)
			{
		...
		/// </summary>
        /// <remarks>This method was added to allow multi user searches</remarks>
        private DataTable GetCreateHitsTable()
		{
            DataTable hits = new DataTable("FT_Search_Hits");
