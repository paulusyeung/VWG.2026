...
namespace HtmlHelp.ChmDecoding
{
	...
	internal class BinaryReaderHelp
	{
		...
		internal static string ExtractUTF16String(ref BinaryReader binReader, int offset, bool noOffset, Encoding encoder)
		{
            ...
            int intLength = name.Length;

            // If is larger then zero and is odd
            if (intLength>0 && (intLength % 2 == 1))
            {
                // If is null terminated remove the last byte
                if (name[intLength - 1] == 0)
                {
                    intLength -= 1;
                }
            }

            // Get the string without the null terminated byte
			strReturn = Encoding.Unicode.GetString(name,0,intLength);
