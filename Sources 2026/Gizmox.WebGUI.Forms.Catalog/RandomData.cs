...
namespace Gizmox.WebGUI.Forms.Catalog
{
    ...
    public class RandomData
	{
        ...
        public class RandomObject
        {
            ...
            private static string[] marrNames = new string[] { "Roger", "David", "Rayn", "Eric", "Stieve", "Jhon", "Mark", "Tom", "Dan"};
            private static string[] marrImages = new string[] { "Add", "Back", "Copy", "Cut", "Folder", "Help"};
            public RandomObject()
            {
                
            }

            public static RandomObject[] GetList(int intCount)
            {
                RandomData objRandomData = new RandomData();

                Color[] objColors = RandomData.GetWebColors();
                List<RandomObject> objList = new List<RandomObject>();

                for (int intIndex = 0; intIndex < intCount; intIndex++)
                {
                    RandomObject objObject = new RandomObject();
                    objObject.mobjColor = objColors[objRandomData.GetInteger(0, objColors.Length)];
                    objObject.mstrName = marrNames[objRandomData.GetInteger(0, marrNames.Length)];
                    objObject.mstrImage  = string.Format("Icons.{0}.gif", marrImages[ objRandomData.GetInteger(0, marrImages.Length)]);
