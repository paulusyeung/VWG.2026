...
namespace HtmlHelp
{
    ...
    public sealed class HtmlHelpSystem
	{
		...
		{
			Storage.FileObject objFileObject = null;
			foreach(CHMFile objCHMFile in this._chmFiles)
			{
				objFileObject = objCHMFile.GetFile(strLocal);
			...
			if(objFileObject!=null)
			{
                string strFile = strLocal.ToLower();
                if (strFile.EndsWith(".gif"))
                {
                ...
                else if (strFile.EndsWith(".htm") || strFile.EndsWith(".html"))
                {
				...
				BinaryReader br = new BinaryReader(objFileObject);
                objResponse.BinaryWrite(br.ReadBytes((int)objFileObject.Length));
				br.Close();
			}
		}
		...
		public string ReadFromFile(string strLocal)
		{
			...
			if(objFileObject!=null)
			{				
				strContent = objFileObject.ReadFromFile();
				objFileObject.Close();
			}
			return strContent;
		}
		...
		public DataTable PerformSearch(string words, int MaxResults, bool partialMatches, bool titleOnly)
		{
			...
			foreach(CHMFile curFile in _chmFiles)
			{
				if(nCnt == 0)
				{
					...
					{
                        if ((dtResult = curFile.FullTextSearchEngine.Search(words, MaxResults, partialMatches, titleOnly))!=null)
						{
				...
				else
				{
					if(curFile.FullTextSearchEngine.CanSearch)
					{
                        ...
                        if ((table = curFile.FullTextSearchEngine.Search(words, MaxResults, partialMatches, titleOnly)) != null)
						{
