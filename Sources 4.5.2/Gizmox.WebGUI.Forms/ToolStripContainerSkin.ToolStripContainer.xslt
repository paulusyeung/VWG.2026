...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:WC="wgcontrols" xmlns:WG="http://www.gizmox.com/webgui">
  ...
  <xsl:template name="tplDrawToolStripContainerAPI">
    <xsl:param name="prmIsEnabled" />
    <xsl:param name="prmControlClass" select="'ToolStripContainer-Control'"/>
  ...
  <xsl:template match="WC:Tags.ToolStripContainer" mode="modContent">
    <xsl:param name="prmIsEnabled" />
    <xsl:call-template name="tplDrawToolStripContainerAPI">
      <xsl:with-param name="prmIsEnabled" select="$prmIsEnabled" />
    </xsl:call-template>
  </xsl:template>

  <xsl:template match="WC:Tags.ToolStripContainer" mode="modScrollbars">
    <xsl:param name="prmContainerElementID" select="@Attr.Id" />
    <xsl:param name="prmScrollbars" select="@Attr.Scrollbars" />
    <xsl:param name="prmArrowsBaseClass" select="'ToolStripContainer-ArrowsClass'" />
  ...
  <xsl:template name="tplDrawToolStripContainerContentAPI">
    <xsl:param name="prmIsEnabled" />
  ...
  <xsl:template name="tplDrawPanelContentAPI" match="WC:Tags.ToolStripContainer" mode="modFrameCenterContent">
    <xsl:param name="prmIsEnabled" />
    <xsl:call-template name="tplDrawToolStripContainerContentAPI">
      <xsl:with-param name="prmIsEnabled" select="$prmIsEnabled" />
    </xsl:call-template>
  </xsl:template>
