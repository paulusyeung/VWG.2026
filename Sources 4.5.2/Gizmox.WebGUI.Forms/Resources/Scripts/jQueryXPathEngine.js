(function () {
	...
	if(mcntIsIE || typeof document.createExpression == "function")
		return;
	...
	NodeUtil = {
		...
		getElementsByTagName : function( node, name) {
			...
			if(nodeName.length==2){
				var nodes = node.getElementsByTagName(nodeName[1]);
				var matchNodes = [];
				var prefix = nodeName[0];
	...
	@*/
		if (attrValue && attrName == 'id' && false /* ::guy*/&& node.getElementById) {
		    node = node.getElementById(attrValue);
		    if (node && test.match(node)) {
			nodeset.push(node);
		    }
		}
		else if (attrValue && attrName == 'name' && false /* ::guy*/&& node.getElementsByName) {
		...
		else if (attrValue && attrName == 'class' && false /* ::guy*/&& node.getElementsByClassName) {
			...
			var nodes = NodeUtil.getElementsByTagName(node, name);
			if (nodes) {
			    var i = 0;
			    while (node = nodes[i++]) {
	...
	/*@cc_on @if (@_jscript)
		this.attrName = attrName; //::guy.toLowerCase();
	@else @*/
	...
	NameTest = function(name) {
	    this.name = name;//::guy .toLowerCase();
	};
	...
	NameTest.prototype.match = function (node) {
		...
		if (type == 1 || type == 2) {
			if (this.name == '*' || this.name == node.nodeName) {
				return true;
			}
			else {
				// Create regular expression from name.
				var strNodeNameRegExp = new RegExp(this.name.replace('*','.+'));
				
				// Return test result.
				return strNodeNameRegExp.test(node.nodeName);
			}
		}
		return false;
	};
	...
	/*@end @*/
		...
		return '' + nd.nodeName; //::guy .toLowerCase();
	...
	var install = function(win) {
	    ...
	    Document.prototype.createExpression = function(expr) {
		return new win.XPathExpression(expr, null);
	    };
	    
	    Document.prototype.evaluate = function(expr, context, _, type) {
