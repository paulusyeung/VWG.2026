...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:WC="wgcontrols" xmlns:WG="http://www.gizmox.com/webgui">
  ...
  <xsl:template name="tplDrawToolStripPanelAPI">
    <xsl:param name="prmIsEnabled" />
    <xsl:param name="prmControlClass" select="'ToolStripPanel-Control'"/>
  ...
  <xsl:template match="WC:Tags.ToolStripPanel" mode="modContent">
    <xsl:param name="prmIsEnabled" />
    <xsl:call-template name="tplDrawToolStripPanelAPI">
      <xsl:with-param name="prmIsEnabled" select="$prmIsEnabled" />
    </xsl:call-template>
  </xsl:template>

  <xsl:template match="WC:Tags.ToolStripPanel" mode="modScrollbars">
    <xsl:param name="prmContainerElementID" select="@Attr.Id" />
    <xsl:param name="prmScrollbars" select="@Attr.Scrollbars" />
    <xsl:param name="prmArrowsBaseClass" select="'ToolStripPanel-ArrowsClass'" />
  ...
  <xsl:template name="tplDrawToolStripPanelContentAPI">
    <xsl:param name="prmIsEnabled" />
  ...
  <xsl:template name="tplDrawPanelContentAPI" match="WC:Tags.ToolStripPanel" mode="modFrameCenterContent">
    <xsl:param name="prmIsEnabled" />
    <xsl:call-template name="tplDrawToolStripPanelContentAPI">
      <xsl:with-param name="prmIsEnabled" select="$prmIsEnabled" />
    </xsl:call-template>
  </xsl:template>
