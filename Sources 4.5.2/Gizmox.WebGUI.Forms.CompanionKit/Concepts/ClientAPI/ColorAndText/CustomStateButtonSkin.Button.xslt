...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="WC:Tags.Button[@Attr.CustomStyle='CustomStateButtonSkin']" mode="modContent">
    <xsl:call-template name="tplDrawButtonAPI">
      ...
      <xsl:with-param name="prmClickHandler">
        mobjApp.CustomStateButton_Click('<xsl:value-of select="@Attr.Id"/>');
      </xsl:with-param>
    </xsl:call-template>
  </xsl:template>

  <xsl:template match="WC:Tags.Button[@Attr.CustomStyle='CustomStateButtonSkin']" mode="modFrameCenterContent">
